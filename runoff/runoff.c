...
void tabulate(void)
{
    // we loop through the voters
    for (int i = 0; i < voter_count; i++)
    {
        for (int j = 0; j < candidate_count; j++)
        {
            int curr_candidate = preferences[i][j];
            if (!candidates[curr_candidate].eliminated)
            {
                candidates[curr_candidate].votes++;
                break;
            }
        }
    }
...
int find_min(void)
{
    int m_value = voter_count;
    //candidate m_candidate;
    for (int i = 0; i < candidate_count; i++)
    {
        if (!candidates[i].eliminated)
        {
            if (candidates[i].votes < m_value)
            {
                //m_candidate = candidates[i];
                m_value = candidates[i].votes;
            }
        }
    }
    return m_value;
    ...
    */
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        if (!candidates[i].eliminated)
        {
            if (first)
            {
                c_val = candidates[i].votes;
                first = false;
            }
            else
            {
                if (candidates[i].votes != c_val)
                {
                    return false;
                }
            }
